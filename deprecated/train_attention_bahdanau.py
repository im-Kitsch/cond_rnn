...
from model.cond_rnn_attention import CondGruAttention
...
cond_attn = CondGruAttention(num_class=NUM_CLASS, hid_size=args.hid_size,
...
model_summary = torchsummaryX.summary(cond_attn, test_dt, test_cond)
...
def show_attention(ax, attention, input_condition, sentence):
...
cond_attn.to("cpu")
with torch.no_grad():
    batch_data, batch_label, batch_cond = training_loader.dataset[[0]]
    batch_data, batch_label, batch_cond = torch.tensor(batch_data).T, torch.tensor(batch_label).T, torch.tensor(batch_cond).T

    _, _, weights = cond_attn(batch_data, batch_cond)
    weights = weights.squeeze(1)

    fig = plt.figure()
    ax = fig.add_subplot(111)
    ax = show_attention(ax, weights.numpy(),
