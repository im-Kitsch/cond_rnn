...
from model.cond_vae import CondVAE
from util.traj_dataset_general import TrajDatasetGeneral, wrap_data
from util.data_prepare import traj_prepare, cond_prepare
from util.nlp_util import trans_one_hot
...
def train(args):
    ...
    seq_vae = CondVAE(num_class=NUM_CLASS, embedding_size=args.embed_size, embedding_drop_out=args.emb_dropout,
    ...
    model_summary = torchsummaryX.summary(seq_vae.model, test_batch['data'], test_batch['cond'])
...
def eval(seq_vae, translator):

    z = seq_vae.model.sample_z(1, device=DEVICE)
    z = z.repeat(15, 1)
    cond = seq_vae.model.sample_cond(15, device=DEVICE)

    pred_traj = seq_vae.model.inference(z=z, cond=cond, pred_length=72, batch_first_z_cond=True, device=DEVICE)
